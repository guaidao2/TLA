...
**2D 推力体世界**（真实世界模型场景，2026-08-11）：状态 $s=(x,y,vx,vy)$（4 维，边界反弹有界）、
动作 $a=(ax,ay)$（2 维推力），预测 $s'$；半隐式速度 + 重力 $g$ 可调 + restitution 反弹。
重力 $g$ 即物理规则——$g$ 变化构成持续学习任务序列（比弹簧的 $\omega$ 更真实的物理参数漂移）。
观测 $=[s,a]$（6 维），目标 $=s'$（4 维）。本地生成、零外部依赖。

### 4.2 判据体系

判据**先于实现**（预注册）：跑数前锁死标准，跑数后只许改代码、不许改判据；负结果如实记录。
全部 75 个测试（pytest）固定种子、可复现。

### 4.3 主要结果

| 判据 | 初测 | 修复后 | 关键数字 |
|---|---|---|---|
| P-LEARN-3 无 BP 学得动 | 通过 | — | 训练 MSE 0.0008；未见 ω 0.480 vs 随机 0.978 vs 恒等 0.670 |
| 学习强度（无捷径弱学习） | 未过 0.11 | 通过 **0.0046**（原则一） | 分布内 MSE，解锁阈值 0.02 |
| P-COG-3 琢磨消融 | 未过 空转/负价值 | 通过 **有能力轴翻转**（双过程回退） | 噪声轴 0.0931 < 瞎猜 0.0966 |
| P-LEARN-1 防遗忘 | 未过 保留率 6.4% | 通过 **108.6%**（突触巩固 EWC） | 无重放 101.9%；学习强度保持 0.0084<0.02 |
| P-COG-4 doubtful 校准 | 通过 | — | 低置信 5.1× 高置信；doubtful 5.3× 未标记 |
| P-COG-1/2 会琢磨步数 | 部分 | — | 干净 median=3（预算 8）；噪声均值差分 2.67 vs 3.36 |
| P-LEARN-2 放大 | 通过 | — | hidden 32→128 成本比 1.07 |
| TC-1/2/3 推力体（s,a→s'） | 通过 | — | 学习 0.0882；动作条件化 0.0500/−0.1827；EWC 98.5% |
| BC-1 基线对比 | 通过（显著赢） | — | TLA 0.0882 vs LSTM 0.2227/ODE 0.2578/SSM 0.2452 |
| AB-1/2/3/4 消融 | 独有机制空转 | — | 推理环 0.0882≈0.0884；回退/Self_Slot 默认关；W_base 1.0× |
...
### 4.6 真实世界模型场景：推力体、基线与消融（2026-08-11）

**2D 推力体世界**（§4.1，本地生成含动作连续控制）：三判据全通过——
TC-1 学习成立（未见重力 0.0882 < 随机 0.4630 < 恒等 0.1166）；
TC-2 动作条件化（配对恒推 +1 → vx'=0.0500 vs −1 → −0.1827，同一状态序列，模型真用动作）；
TC-3 防遗忘（g1.0→g2.0 重力任务序列，EWC 保留率 98.5%）。

**基线对比**（同种子同分割，BC-1 PASS）：TLA（无 BP）0.0882 vs
LSTM 0.2227（赢 2.5×）、NeuralODE（手写 RK4）0.2578（赢 3.0×）、Mamba 式对角 SSM 0.2452
（赢 2.8×）。敏感性：基线给 5× 训练（10 epoch）仍在追（LSTM 0.122 等）——"没训够"排除。
诚实披露：① TLA 含摊销线性捷径（W_base），基线无捷径——赢的方向可能部分来自捷径归纳偏置；
② TLA infer 含 settle 迭代（测试时多步精化），基线单次前向——测试时算力不对称；
③ 单 seed 无方差；参数量同量级（TLA ~700 vs LSTM 3772/ODE 1088/SSM 320）。

**消融**（逐件关掉独有组件，AB-1/2/3/4）：推理环**空转**（纯首猜 0.0884 ≈ 自适应 0.0882，
承重证据 g0≈ad：完全跳过 settle 也不降；fixed1≈ad 部分是结构性早停）；
双过程回退与 Self_Slot **默认关**（贡献=0 由配置/代码结构决定，如实披露非实验）；
摊销捷径 W_base 冻结在 init 后标准量 1.0× 无差别、轻量 5.4% 轻微（残差通路基本独力学会，
不同于弹簧先例 27× 关键性）。

**净结论（关键）**：TLA 赢基线的 2.5-3× 来自**残差 PCN + LTC 基板 + 无 BP 局部学习**组合本身；
琢磨/双过程回退/Self_Slot 在平滑预测任务上**全是摆设**（消融判据的直接回答——"独有机制
没起作用"）。独有机制的用武之地需事件/计时/非确定性任务（奇点-薛定谔方向），平滑预测
不是它们的场地。
...
**摊销首猜 × MoE v2（2026-08-11 实测，代码已回滚、结论保留）**：每专家=完整摊销预测器
（自己的 W_base/W_1/W_out），路由=暖状态**观测部分**重建误差 + 使用率平手打破对称。
四实验裁决：
- **E1 学习强度：PASS**（标准量 0.0066）——首猜承重治愈 MoE 弱学习（本组合的核心正结果）；
- **E2 防遗忘：未达成（负结果锁死）**——路由从未真正分离 A/B（B 域 50/50、usage 均衡），
  结构性腿 29.3% 不达标，EWC 腿标准量 19.5%（轻量 102.6% 为假阳）；
- **E3 琢磨：无稳健增益**（标准量 0.3197 vs 固定 0.3196 死平；步数无差分）——与全部历史一致；
- **E4 路由分离：PASS 但部分**（A 域 71-74% 单专家，B 域 50/50）。
**归因**：共享 LTC 状态（16 维 h）主导 [s,h] 输入空间，重建路由信号不具任务判别性；
分离收益必须建立在可判别路由上——"消遗忘"依赖的路由判别性在当前基板下未达成。
换真实任务（可判别状态空间）前，此负结果成立。
...
5. **摊销×MoE 已实测（2026-08-11）**：E1 学习强度 PASS（0.0066，首猜承重治愈弱学习）；
   但 E2 防遗忘未达成（路由不判别，EWC 腿标准量 19.5%）、E3 琢磨无稳健增益（死平）——
   共享 LTC 状态使 [s,h] 空间的路由信号不具任务判别性（§5.3）。
   **下一步**：换真实小任务（可判别状态空间：图像/语言/控制），验证"琢磨 + 防遗忘"
   组合在判别性状态上的表现；若仍失败，则确认路由判别性是硬前置。
...
P-META-1~4（生长/校准/修剪/Self_Slot）。全部 75 个测试见 `tests/`，裁决见 §4.3 与设计文档 §10。
...
*本文所有实验数字均可由仓库代码复现（固定种子，pytest 75/75）。负结果与降级分支是判据的一部分，同样锁死。*
