...
预注册判据（2026-08-11 阈值校准，方向未变）：
  SW-5a 回切保留：gated 的回切恢复成本 ≤ 0.95 × uniform 的（gated 保留旧规则假设 →
     回切快恢复；uniform 全可塑，失败分支漂移后回切恢复慢）。
  ...
  frozen（真 η=0）作为极端对照记录（终审修复：初版 frozen 落到 uniform 分支，
  "uniform≈frozen 饿死效应"是 bug 产物已撤回；修复后 frozen 切换成本最低
  ——不适应+坍缩选最不差分支，但无新规则适应能力，靠分裂）。
