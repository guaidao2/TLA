...
class AmortizedMoEPCN:
    ...
    def __init__(self, dims, out_dim, n_experts=2, lr_inf=0.1, prior=0.0,
        ...
        self.winner = 0
        self._routed = False                            # 是否已路由过（readout/guess 防重路由）
    ...
    #      打破对称（冷启动交替、防死专家）。纯硬路由（v1 的 0.8/0.2 混合是残留，v2 只赢家学）。----
    def _route(self, x, update_proto=True):
        ...
        self.last_routing = torch.softmax(-torch.tensor(errs) / e_min, dim=0)
        self.winner = hard_idx
        self._routed = True
        return self.last_routing, hard_idx
    ...
    # ---- 推理（只赢家 settle；读出一致性：readout/guess 不重路由）----
    ...
    def readout(self, x):
        # 用最近一次路由的赢家（settle/learn 已定），不重路由——同一推理步内
        # settle 与读出一致（防读出败者输出，终审 should-fix）
        if not self._routed:
            self._route(x, update_proto=False)
        return self.expert[self.winner].readout(x)

    def guess(self, x):
        """系统1：赢家专家的摊销首猜（双过程回退的兜底输出）。"""
        if not self._routed:
            self._route(x, update_proto=False)
        return self.expert[self.winner].W_base @ x + self.expert[self.winner].b_base
    ...
    def learn_step(self, x, target, lr=0.01, settle_steps=4, wd=1e-4,
        ...
        self.last_max_err = ex.last_max_err            # 学习路径也上报（scratchpad 诊断）
