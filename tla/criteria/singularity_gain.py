...
实测裁决（2026-08-11，n_traj=20/T=30/n_ep=2，动力学修复后）：
  ...
  S-T3 更快动力学改善一步预测（正方向，余量薄）：修复（λ=0.5、β=3.0 → 衰减相出现）
     后奇点 0.1232 vs LTC 0.1346，多种子比 [0.915, 0.998] 全 <1——但 seed2 余量仅
     0.2%（0.998）、57 样本/种子点估计无误差带；**"时间戳可解码增益"为假说未验**：
     S-T2 双簇仍 FAIL、无任何测试真实解码时间戳、W_h 惰性（递归记忆≈0）下"修复=更接近
     LTC 连续分布"的伪增益解释未被排除——只可断言"更快动力学改善一步预测"；
  S-T4 衰减相出现（前提而非已验）：热后衰减事件 688——只证明"热细胞撤驱动即快速回落"
     （λ=0.5 自由衰减≈0.45/tick，度量平凡可满足），τ_decay=2 vs T=30 时间戳分辨率
     ≤2-3 tick，"可解码"未验。
重开记录：首轮 S-T3 按"无增益"方向锁负（旧比 1.036，判据 ≤1.1× 下本为 PASS——当时
按其与 LTC 无显著差异记录）；因实现缺陷（衰减相缺失）重开，断言从 ≥0.9×LTC 改为
≤1×LTC；判据标准（S-T1/T3 阈值）未改，S-T3 方向性结论修正。

判据锁死：S-T1/T3 与 S-T2 的 inact 腿零改动；S-T2 的 act 腿由恒真式（≥0.5）修正为有
约束力的真炸要求（>0.7）——修正使判据可识别贴边假双簇，裁决方向不变。标定参数
（eps/input_scale）是测试设置，判据标准不变。
"""
import torch
